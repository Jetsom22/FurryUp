...
```
타이틀 (Press to AnyKey) → 로비
  로비: 상단 바(아이콘 · 재화 자리 0 고정 · 설정) / 메인 배너 / 사용 캐릭터 정보 / 임시 배너 · 도감 · 캐릭터 선택 · 게임 시작
  도감: 육/해/공 · 역할군 5종 필터, 6열 카드(42종, 잠금 12종은 쇠사슬+자물쇠), 카드 클릭 시 정보 팝업
  캐릭터 선택: 잠금 해제 30종 5열 카드, 장착 → "사용 캐릭터" 배지, 우측 정보 패널 (장착은 localStorage 에 저장)
  게임 시작 → 캐릭터 선택(장착 확인) → 챕터 선택(이전/다음, 1챕터만 열림) → 게임 시작
  → 장착 캐릭터가 편성 1번으로 들어가 1-1 바로 진입 (별도 편성 화면 없음)
  → [스테이지] 스토리 / 이벤트 / 전투(일반·중간보스·보스, 진입 시 자동 시작)
  → 클리어 → 상점 (상품 5칸 구매 · 덱 구성 · 인벤토리) → 다음 스테이지
  → … → 1-15 보스 클리어 → 로비  /  전멸 → 로비
```

아웃게임 5화면(타이틀 · 챕터 선택 · 도감 · 로비 · 캐릭터 선택)과 전투 · 상점(덱 구성 · 인벤토리)은 UI 개편 와이어프레임을 1920×1080 으로 옮긴 것입니다.
로고 · 배너 · 챕터 이미지 · 아이템 아이콘은 자리만 잡아 두었습니다.

### 전투 화면
- 좌상 스테이지 진행 바(▲ 스토리 ● 일반 ○ 이벤트 ◆ 중간보스 고양이 = 보스, 빨간 점 = 현 위치), 상단 "현재 턴 | N", 우상 설정
- 우측 적 파티 목록(초상 + 세로 HP 바, 클릭하면 앞장으로), 중앙 겹친 적 카드의 **앞장 = 지금 행동하거나 피격 중인 적**
- 하단 아군 카드 5장: 초상 · 이름 · HP · 세로 투지 바(스킬 요구량 칸) · 모서리 역할군 아이콘 · 상태 칩(▲버프 ▼디버프 독/출혈/기절…)

### 상점 (정비)
- 상품 5칸 = 캐릭터 3 + 아이템 2. 상품 위에 마우스를 올리면 정보 패널, 클릭하면 상점 주인 말풍선에서 구매 확인. 리롤 없음
- 캐릭터 구매 → 빈 칸이 있으면 자동 배치, 덱이 꽉 차면 덱 구성에서 [교체] / [교체하지 않는다](대기 명단)
- 덱 구성 팝업: 현재 덱 5칸(LV 배지) · 카드 선택 → [교체] → 다른 칸 [교체]로 위치 교체 · 전투불능 카드는 [부활 50G] · 빈 칸은 대기 명단에서 [배치]
- 인벤토리 팝업: 아이템 카드 위에 마우스를 올리면 [사용] → 덱 구성에서 캐릭터 [선택]
- 종료 버튼 = 런 포기(말풍선에서 확인) → 로비

### 레벨 · 아이템 (임시 설계, `data/balance.js` LEVEL / ITEMS)
- 레벨 1~10. 다음 레벨 필요 EXP = 10 × 현재 레벨. 레벨업마다 체·공·방·속 각 +1 (체력 +1 = 최대 HP +10, 현재 HP 도 +10)
- 아이템: 경험의 열매(EXP +10, 60G) · 황금 경험의 열매(EXP +30, 150G) · 들판의 약초(HP 50% 회복, 50G). 전투불능에는 회복약 사용 불가
...
### 정비 화면 (정비 화면 기획서 → UI 개편 상점)
- 구현: 상품 5칸(캐릭터·아이템), 덱 구성(배치·위치 교체·부활 50G), 인벤토리, 골드, 다음 스테이지, 맵 대신 스테이지 진행 바. 상인은 안내자(CHR_901)
- **미구현(범위 외)**: 유물, 버프, 칸 강화, 리롤(개편에서 제거)
...
| `data/balance.js` | `RULES`: 실험실 설정값(HP 배수, 공격 계수, 최소 피해, 공통 스킬, 투지 획득, 회피율, 최대 턴) + 역할군 효과 + 적 배율·골드·상점 진열·레벨·아이템·이벤트 | ○ |
| `data/stages.js` | 챕터별 이름·설명·스테이지 수·고정 배치·랜덤 구간, 이벤트 확률/제약, 스토리 대사 | ○ |
...
적 체력·공격 = 실험실 값 × 0.7 × (1 + 0.065 × (스테이지 − 1)), 중간보스 HP×1.5, 보스 HP×2.0, 방어는 배율 없음 (`data/balance.js` ENEMY).
...
src/core/             렌더링과 분리된 순수 로직
  ...
  run.js              런 상태 (보유/편성/레벨·EXP/인벤토리/골드/상점/부활/이벤트 효과)
src/ui/theme.js       공용 UI 헬퍼
src/ui/widgets.js     공용 위젯 (상단 바 / 카드 / 스크롤 영역 / 캐릭터 정보 패널 / 스테이지 진행 바 / 상태 칩 / 아이템 아이콘 / 장착 저장)
src/flow.js           씬 전환 규칙
src/scenes/           Boot / Title / Lobby / Codex(도감) / CharacterSelect / ChapterSelect / Battle / Story / Event / Shop
...
node tests/engine.test.js
```
데이터 변환, 스테이지 배치 제약, 피해식, 행동 순서, 대상 선택, 역할군 5종, 공통 스킬, 문서 반영 패시브·스킬 7종, 42종 실데이터 전투, 상점/부활/편성 교체/이벤트, 레벨·EXP·아이템을 검증합니다.
