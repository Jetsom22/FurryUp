...
타이틀 (Press to AnyKey) → 로비
  ...
  → [스테이지] 스토리 / 이벤트 / 전투(일반·중간보스·보스)
  → 클리어 → 정비 화면 (상점에서 캐릭터 구매 · 배치 · 부활 · 리롤 · 맵) → 다음 스테이지
  → … → 1-15 보스 클리어 → 로비  /  전멸 → 로비
```

아웃게임 5화면(타이틀 · 챕터 선택 · 도감 · 로비 · 캐릭터 선택)은 UI 개편 와이어프레임을 1920×1080 으로 옮긴 것입니다.
로고 · 배너 · 챕터 이미지는 자리만 잡아 두었습니다.
...
src/ui/widgets.js     아웃게임 위젯 (상단 바 / 카드 / 스크롤 영역 / 캐릭터 정보 패널 / 장착 저장)
src/flow.js           씬 전환 규칙
